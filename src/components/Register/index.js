...
export default function SignUp() {
  ...
  const onSubmit = (data) => {
    if(verifyData(data)){
      console.log(data)
      setMessage({type: 'success', detail: 'Usuario creado con éxito. Te enviamos un mail para que puedas activar tu cuenta.'})
    }
  };
  ...
  const verifyData = (data) => {
    ...
    if (!email) {
      setMessage({type: 'error', detail: 'Por favor, completa la dirección de correo electrónico'});
      return false;
    }
    ...
    if (!password) {
      setMessage({type: 'error', detail: 'Por favor, completa la contraseña'});
      return false;
    }

    if (!confirmPassword) {
      setMessage({type: 'error', detail: 'Por favor, completa la verificación de contraseña'});
      return false;
    }
    ...
    if (!codepost) {
      setMessage({type: 'error', detail: 'Por favor, completa el Código Postal'});
      return false;
    }

    if (!streetNumber) {
      setMessage({type: 'error', detail: 'Por favor, completa el número de dirección'});
      return false;
    }
